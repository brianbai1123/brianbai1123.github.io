(function () {
  ...
  var here = location.origin + location.pathname + location.hash;
  ...
  fetch("/crosslinks.json")
    ...
    .then(function (links) {
      var list = links[here] || links[here.replace(/\/$/, "")] || links[here.replace(/\/#/, "#")];
      if (!list || !list.length || document.querySelector("[data-reading-room]")) return;
